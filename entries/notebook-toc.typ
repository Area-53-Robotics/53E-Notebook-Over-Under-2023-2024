...
#create_entry(
  ...
  In past years our digital notebooks suffered from one major problem: the table of contents was very difficult to maintain. If something in the introduction was added, it would change every page number of every entry, which would then need to be updated in the table of contents, for every entry, which was usually a non-trivial amount of work, usually over an hour. We've solved this problem in this notebook by automatically generating the table of contents.

  The first thing we did was define a global variable to store all of our entries in:
  ...
  ```
  Each entry is represented as a dictionary, which assigns key value pairs to all of its relevant data, including title, type, start date, end date, and the actual content of the entry.

  Then we have a function that adds entries to the table of contents for every entry in that global variable.

...
#create_entry(
  ...
  2. Place the robot so that the front of the drivetrain is against the edge of the tape.
  3. Have another teammate ready with a camera, and record the next step.
  4. Drive the drivetrain as fast as possible forwards until the front of the drivetrain touches the other tape.
  ...
  Overall the power draw held very steady for both sides, but this is expected since the wheels were free spinning.
  ...
  #grid(
    ...
    The left motors power draw is very distributed. The 3rd motor is using a lot of power, staying around 2W. The 2nd motor operates at around the expected wattage, at about 1W, and the first motor uses almost no power at all.

    ],
    image("../assets/drivetrain/tests/power-left.png"),
    [
    #nb_heading([Right Motors Power Draw], level: 3)
    The right side motors are much less distributed. The 1st and 2nd motors use under 1W, but the third motor uses around 1.35W.
    ],
    ...
    #nb_admonition(
      ...
      The temperature sensors on the V5 motors only output the temperature in steps of 5$degree.c$.
      ]
    Due to the short length of the test and the low accuracy of the temperature sensors the results of this test are almost completely useless.
    After about 30 seconds the 3rd motor increases from 35$degree.c$ to 40$degree.c$, and that the 1st and 2nd motors stay stagnant at 40$degree.c$.
    We can see that about 30 seconds in the temperature changes by 5$degree.c$. We can conclude that the decreased temperature of the 3rd motor is due to its low power draw but it's likely that if we want to get meaningful data from this we will need to test over a much larger period of time.
    ],
    image("../assets/drivetrain/tests/temperature-left.png"),
    [
    #nb_heading([Right Motors Temperature], level: 3)
    This test fared similarly to the left side motors. The 1st motors stayed stagnant at 40$degree.c$. The 2nd and 3rd motors started at 35$degree.c$ and increased to 40$degree.c$ over time, the 2nd motor after 10s and the 3rd motor after 30.
    ],
  ...
  #nb_admonition(
    ...
    The video we took of the 1st trial started late, and therefore did not have enough info to calculate the correct time.
    ]
  ...
  We also recorded the velocity of the left and right side of the drivetrain during each test.

  #let graph_width = 75%

  #figure(caption: [ Trial 1 ], image(
    "../assets/drivetrain/tests/test-1.png",
    width: graph_width,
  ))
  #figure(caption: [ Trial 2 ], image(
    "../assets/drivetrain/tests/test-2.png",
    width: graph_width,
  ))
  #figure(caption: [ Trial 3 ], image(
    "../assets/drivetrain/tests/test-3.png",
    width: graph_width,
  ))
  #figure(caption: [ Trial 4 ], image(
    "../assets/drivetrain/tests/test-4.png",
    width: graph_width,
  ))
  #figure(caption: [ Trial 5 ], image(
    "../assets/drivetrain/tests/test-5.png",
    width: graph_width,
  ))
  ...
  The drivetrain matches our projected speed of 5.96 f/s pretty closely with an average speed of 5.76 f/s. It is also much faster than our drivetrain from last year, properly taking advantage of all 6 motors.

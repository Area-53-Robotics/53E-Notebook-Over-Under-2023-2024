...
We've set up the flywheel to be toggled with the L2 button, replacing the catapult.
```cpp
// Flywheel control
...
```
This is overall easier for the driver, it means they don't have to think about holding down a button the whole time the flywheel is running.

This PID output can be graphed with the Loginator. This allows for easy debugging and testing.
#figure(
  image("./loginator-output.png"),
  caption: [Grafana, displaying PID output from the flywheel]
)

The dips represent the output of the controller as the flywheel was toggled on and off.

= LED Indicator

When we were match loading during our Hereford tournament we noticed that it was very difficult for the match loaders to know when the flywheel was back up to speed again. This lead to very inconsistent trajectories.

We decided to add an LED indicator light to show the match loaders when the flywheel was spinning fast enough.

Implementing this was simple enough, Sylib also provides built in LED control. These 5 lines of code were all we needed to get this working.
```cpp

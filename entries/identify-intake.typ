...
#create_entry(
  ...
  In order to be able to manipulate the triballs inside our robot, we will first need to bring them inside our robot. While these are normally extremely essential to every season, this season they are less important due to the fact that triballs can be loaded directly into the robot when its touching the match load zone.

  #align(
    center,
  )[
    #image(
      "../assets/game-challenges/picking-up-triballs.svg",
      height: 25%,
    )
    ]

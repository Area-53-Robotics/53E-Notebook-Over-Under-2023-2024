...
In order to get the position tracking working we first had to install LemLib. We followed the installation instructions #footnote(link("https://lemlib.github.io/LemLib/md_docs_tutorials_1_getting_started.html")) and downloaded the zip file for the 0.4.7 release. Then we ran the following commands to apply the template:
```shell-unix-generic
...
Then we had to set the constants for the PID #footnote("See glossary") loops. We'll cover what this means in more detail when we need to solve moving the robot during the autonomous period.
...
```
Once all of this was done we could combine it all together with the chassis constructor.
```cpp
// create the chassis
...
Now that the setup is complete all we have to do is call the chassis's calibrate method and we're good to go.
```cpp
void initialize() {
  chassis.calibrate();
}

...
The most important part of LemLib's moveToPoint algorithm is it's PID implementation. As mentioned before, PID is a closed loop controller, which means that it corrects for external forces. The output of the loop is calculated every 20 ms with the following equation:
...
`AngularPower` and  `LinearPower` are simply the output of the two PID loops. LemLib calculates the error with some simple trigonometry.
...
#nb_admonition(type: "equation")[
Assuming the following variables:
- $d_"lead"$ is a constant set by us that dictates how far away the carrot point is from the end point.

...
#decision-matrix(
  properties: ((name: "Simplicity"), (name: "Accuracy"), (name: "Robustness")),
  ("Bang Bang", 3, 1, 1),
  ("PID", 2, 3, 3),
...
float lemlib::FAPID::update(float target, float position, bool log) {
  ...
  if (kA != 0)
    output = lemlib::slew(output, prevOutput,
                          kA); // Limit rate of change, if applicable
  prevOutput = output;
  prevError = error;
...
Overall this has a lot of advantages over a simple move to point. It means that
the system only has to settle once, and it also means that the system knows what
angle it should end at, giving it more certainty at the end of movements.

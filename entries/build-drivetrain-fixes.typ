...
#create_entry(
  ...
  After testing our drivetrain and discovering several issues with it, we began to fix them.

  #grid(
    columns: (1fr, 2fr),
    gutter: 20pt,
    [
    The first issue we fixed was structural. We noticed that the spacers used as boxing for the C-channel were not snug, and were able to spin freely. This meant that they weren't taking load, and were effectively useless. We fixed this by finding the correct spacer configuration, which was added up to 0.875". We then replaced the existing spacers with white nylon ones.
    ],
  ...
  #grid(
    ...
    The second issue was also structural. The ends of the C-Channel on our drivetrain were only held together by a 1.5" standoff and a 0.375" nylon spacer. We decided to double this in order to increase the squareness of the drive.
    ],
  )
  ...
  #grid(
    ...
    gutter: 20pt,
    [
    The third issue had to do with the motor inserts coming out while driving. Our old configuration involved just a single motor insert, but the space between the motor cartridge and the C-channel is too large, and therefore allows the insert to come out, disengaging the motor from the axle.

    We solved this problem by adding an extra 1/16" spacer between the insert and the C-channel, and that solved the issue.
    ],

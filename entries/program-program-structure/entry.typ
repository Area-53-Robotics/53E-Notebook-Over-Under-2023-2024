...
During Spin Up we had to have a large amount of boilerplate for each state
machine we wanted to create. In order to fix this problem we've made an abstract
StateMachine class that the subsystems can inherit. This means that we can
define the state machine structure once, and then use it in a bunch of other
places.

This solution was inspired by Theo from team 7842B #footnote(
  link("https://theol0403.github.io/7842B-Journal/2019-10-16/statemachine/"),
), and Ryan from team 4253B #footnote(
  link(
...
This solution uses a mutual exclusion #footnote("See glossary.") (mutex) in
order to restrict access to the state to one task at a time. This makes the
state safe from data races, in which multiple tasks race to read or modify the
same variable.
...
template <typename State, State init_state = State::Idle> class StateMachine {
private:
...
is also inspired by Theo. #footnote(
  link(
...
class Catapult : public lib::StateMachine<CatapultState>,
                 public TaskWrapper { // Inherit both classes here
public:
  Catapult(std::shared_ptr<pros::Motor>
               i_motor); // We take a pointer to the motor instead of a port to
                         // avoid having to configure it
  ~Catapult();
